...
class YamlSmokeTests(unittest.TestCase):
    ...
    def test_validates_yaml_recursively(self):
        cli.validate_yaml(
            path.join(TEST_DATA_PATH, 'repo.json'),
            TEST_DATA_PATH
        )
    ...
    def test_handle_untested_marker(self):
        cli.list_region_tags(
            path.join(self.smoke_path, 'repo.json'),
            self.smoke_path,
            False,
            False,
            False,
...
class TestedFalseClauseTests(unittest.TestCase):
    ...
    def capsys(self, capsys):
        ...
        cli.validate_yaml(
            path.join(invalid_path, 'repo.json'),
            invalid_path
        )
...
class ExplicitlySpecifiedClauseTests(unittest.TestCase):
    ...
    def capsys(self, capsys):
        ...
        cli.validate_yaml(
            path.join(explicit_path, 'repo.json'),
            explicit_path,
        )
        out, _ = capsys.readouterr()
