...
exports.getTopLevelMethods = (sourceTokens, sourcePath) => {
  ...
  const snippetMethods = parsers.directInvocation.parse(
    sourceTokens,
    sourcePath
  );
  const wrappedMainMethods = parsers.wrappedMain.parse(
    sourceTokens,
    sourcePath
  );
  const wrappedDirectInvocationMethods = parsers.wrappedDirectInvocation.parse(
    sourceTokens,
    sourcePath
  );
  let allMethods = [
  ...
  const allMethodNames = allMethods.map(
    (m) => m.drift.methodName || m.drift.functionName
  );
  allMethods.forEach((m) => {
    m.drift.children = getMethodChildren(m).filter((c) =>
      allMethodNames.includes(c)
    );
  });
...
exports.getCliCommands = (sourceTokens) => {
  ...
  cliStatement = sourceTokens.body
    .filter(
      (t) =>
        t.type === "ExpressionStatement" || t.type === "VariableDeclaration"
    )
    .slice(-1)[0];
  if (cliStatement && cliStatement.declarations) {
    cliStatement = cliStatement.declarations[0].init;
  }

  let cmdExpression =
    cliStatement && (cliStatement.expression || cliStatement.callee);
  if (!cmdExpression) {
