...
const __getFiles = (dir, predicate) => {
  ...
  const folders = paths.filter(
    (p) => p.isDirectory() && p.name !== "node_modules"
  );

  let files = paths.filter(predicate).map((p) => path.join(dir, p.name));
  folders.forEach((f) => {
...
const getNodeJsFiles = (dir) => {
  return __getFiles(
    dir,
    (p) => !p.name.startsWith(".") && p.name.endsWith(".js")
  );
};
...
const getRegionTags = async (dir) => {
  const { stdout } = await execa("grep", [...REGION_TAG_GREP_ARGS, dir], {
    shell: true,
  });
  const regionTags = uniq(
