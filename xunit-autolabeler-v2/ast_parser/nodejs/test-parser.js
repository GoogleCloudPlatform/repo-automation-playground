...
exports.getDescribeClauses = (testTokens) => {
  const clauses = testTokens.body.filter(
    (s) =>
      s.expression &&
      s.expression.callee &&
      s.expression.callee.name === "describe"
  );
  return clauses;
};
...
const parseItClause = (
  ...
  if (
    !itClause.expression ||
    !itClause.expression.callee ||
    itClause.expression.callee.name !== "it"
  ) {
    return {};
  }
  ...
  // Parse tests
  parsers.httpRequest.parse(
    testStatements,
    commandToDescribeMap,
    describeChain,
    testPaths
  );
  parsers.cloudFunction.parse(
    testStatements,
    commandToDescribeMap,
    describeChain,
    testPaths
  );
  parsers.directInvocation.parse(
    testStatements,
    commandToDescribeMap,
    describeChain,
    testPaths
  );
  parsers.execSync.parse(
    testStatements,
    commandToDescribeMap,
    describeChain,
    testPaths
...
exports.getDescribeChainMap = (
  ...
  describeClauses.forEach((c) => {
    ...
    if (
      c.expression &&
      c.expression.callee &&
      c.expression.callee.name === "describe"
    ) {
      // ASSUMPTION:
    ...
    Object.keys(resultCliMap).forEach((key) => {
      aggregateCliMap[key] = (aggregateCliMap[key] || []).concat(
        resultCliMap[key]
      );
    });
  });
...
exports.addDescribeChainToMethods = (
  testPath,
  methodsWithDriftData,
  describeChainMap,
  fileWideInvocation
) => {
  ...
  methodsWithDriftData
    ...
    .forEach((d) => {
      d.nodejs = d.nodejs || {};
      d.nodejs.testData = d.nodejs.testData || [];
    });
  ...
  methodsWithDriftData.forEach((method) => {
    ...
    if (d.cliInvocation) {
      ...
      describeObjs =
        describeChainMap[d.urlPath.toLowerCase()][d.httpMethod.toLowerCase()];
      ...
      d.functionName &&
      describeChainMap["/" + d.functionName.toLowerCase()]
    ) {
    ...
    if (describeObjs) {
      d.nodejs.testData = d.nodejs.testData.concat(describeObjs);
    }
  });
  ...
  methodsWithDriftData
    ...
    .forEach((d) => {
      // Remove duplicates
      d.nodejs.testData = uniq(d.nodejs.testData, (x, y) =>
        deepEqual(x, y) ? 0 : 1
      );

      // Use language-agnostic test data parameters in exported JSON
      d.testMethods = d.nodejs.testData.map((data) => {
        return {
          className: data.describeChain.slice(0, -1).join(" "),
          name: data.describeChain.slice(-1)[0],
        };
      });
    });
};
